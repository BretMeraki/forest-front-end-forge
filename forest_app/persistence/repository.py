...
try:
    ...
    from forest_app.persistence.models import MemorySnapshotModel, TaskFootprintModel, ReflectionLogModel, UserModel
except ImportError:
...
class MemorySnapshotRepository:
    ...
    def __init__(self, db: Session):
        ...
        # Check if models were imported correctly
        if not hasattr(MemorySnapshotModel, 'user_id'): # Check an expected attribute
            raise ImportError("MemorySnapshotModel appears to be incompletely imported.")
        self.db = db
...
class HTATreeRepository:
    """Repository for managing HTATreeModel persistence with idempotency support."""
    ...
    def __init__(self, db: Session):
        ...
        self.db = db
        self.model = HTATreeModel
    
    def get_tree(self, tree_id: Union[str, UUID], user_id: Union[str, UUID]) -> Optional[Any]:
        """Retrieves a tree by ID for the specified user."""
        try:
            from forest_app.persistence.models import HTATreeModel
            return self.db.query(HTATreeModel).filter(
                HTATreeModel.id == tree_id,
                HTATreeModel.user_id == user_id
            ).first()
        except SQLAlchemyError as e:
            logger.error(f"Database error retrieving tree {tree_id}: {e}")
            raise
    
    def find_by_metadata(self, user_id: Union[str, UUID], metadata_key: str, metadata_value: Any) -> Optional[Any]:
        """Finds a tree by metadata key/value for the specified user.
        This is used for idempotency checks when creating trees.
        ...
        try:
            ...
            if dialect == "postgresql":
                ...
                trees = self.db.query(HTATreeModel).filter(
                    HTATreeModel.user_id == user_id,
            ...
            for tree in trees:
                if tree.manifest and tree.manifest.get(metadata_key) == metadata_value:
                    return tree
            return None
        except SQLAlchemyError as e:
            logger.error(f"Database error finding tree by metadata: {e}")
            raise
    
    def update_metadata(self, tree_id: Union[str, UUID], manifest: Dict[str, Any]) -> bool:
        """Updates the manifest for a specific tree.
        This is used to store idempotency keys and custom data.
        ** Commits the transaction. **
        """
        try:
            from forest_app.persistence.models import HTATreeModel
            tree = self.db.query(HTATreeModel).filter(HTATreeModel.id == tree_id).first()
            if not tree:
                return False

            # Update manifest
            tree.manifest = manifest
            # Mark as modified since it's a JSON/JSONB field
            flag_modified(tree, "manifest")
            ...
            self.db.rollback()
            logger.error(f"Database error updating tree manifest: {e}")
            raise
